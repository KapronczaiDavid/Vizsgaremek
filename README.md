Daizy clean (auto kozmetika) 
A weboldal célja: 

az autókozmetikai szolgáltatásainak professzionális bemutatása, az online időpontfoglalás egyszerűsítése, valamint új ügyfelek szerzése.
Az oldal legyen modern, gyors, mobilbarát és könnyen használható. 

Oldalszerkezet: 
  Navigációs sáv
  logó 
  Autókozmetika fül 
  Fényszóró polírozás fül 
  Járműfertőtlenítés fül 
  Kárpittisztítás fül 
  Árlista fül 
  Időpontfoglalás fül 
  Kapcsolat fül 
  A navigáció minden oldalon legyen elérhető.

Főoldal 
A főoldalon jelenjen meg: 
  Nagyméretű háttérkép egy tiszta autóról 
  „Foglalj időpontot” gomb 
  Rövid bemutatkozás 
  A cég előnyei
  Professzionális eszközök 
  Minőségi tisztítószerek 
  Gyors munkavégzés 
  Megbízható szolgáltatás 

 Szolgáltatások 
Minden szolgáltatás tartalmazz: 
  Egy rövid ismertetőt 
  Munka tartalama  
  Várható időtartam 
  Ár 
Szolgáltatások: 
  Autókozmetika 
  Kárpittisztítás 
  Jármű Fertőtlenítés 
  Fényszóró polírozás 

Árlista 
Legyen áttekinthető táblázatban. 
Előtte–utána galéria 
A weboldalon szerepeljen egy galéria, amely bemutatja az elvégzett munkákat. 

A galériá: 
nagy felbontású képeket  tartalmazzon az előző munkákról 
mobilon is jól használható   legyen 
képek nagyíthatóságára is adjon esélyt 

Technikai követelmények
Reszponzív kialakítást Bootstrap 5 keretrendszer használatával probáljuk meg elérni hiszen ezt régebben használtuk.
Node.js és Express segítségével REST APIt készitunk 
Adatbázis és az időpontfoglalások tárolása MySQL adatbázisba lesz eltárolva
Tiszta, jól dokumentált forráskód
 
